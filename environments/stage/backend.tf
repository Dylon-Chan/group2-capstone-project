terraform {
  ...
  backend "s3" {
    ...
    key     = "environments/stage/deploy.tfstate" #Defines the key (path) within the S3 bucket where Terraform will store the state file for this specific environment
